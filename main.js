...
class song {
  constructor(name, picture, link) {
    ...
    this.song = function play_song() {
      main.innerHTML = `<ul><li><img src="${this.picture}" alt="${
        this.picture
      }"</li>
      ...
      <h2 id='temps'>temps restant: ${temps}</h2>`;
    };
  }
}
let score = 0;
...
let timer = setInterval(function() {
  if (temps > 0) {
    temps--;
    let timezer = document.querySelector("#temps");
    timezer.innerText = `temps restant: ${temps}`;
  }
}, 1000);
